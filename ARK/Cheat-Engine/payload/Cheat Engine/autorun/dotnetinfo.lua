...
settings.ColorStaticBackground=tonumber(settings.s['ColorStaticBackground']) or clAqua

dotnetinfosettings=settings --in the previous version settings wasn't local, but I can not leave it like this as other tools might use 'settings'
...
local function getClassFields(Class)
  ...
  if Class.Image.Domain.Control==CONTROL_MONO then  
    ...
    if isGeneric==nil then return end
    ...
    for i=1,#fields do
      ...
      e.Address=fields[i].staticAddress      
...
local function StartClassFetch(frmDotNetInfo, Image, Filter, FetchDoneFunction)
  ...
  if Image.Classes==nil then
    ...
    frmDotNetInfo.ClassFetchThread=createMonoThread(function(t)
      --get the classess
...
local function ClassListDataRequest(frmDotNetInfo, sender, listitem)
  ...
  if frmDotNetInfo.FilteredClassList then    
    local FilteredClassListEntry=frmDotNetInfo.FilteredClassList[listitem.index+1]
    if FilteredClassListEntry then
      listitem.Caption=FilteredClassListEntry.FullName
    else
      listitem.Caption='<FilteredClassListEntry==nil>'
    end
  else 
...
local function ImageSelectionChange(frmDotNetInfo, sender)
  ...
  if sender.ItemIndex>=0 then 
    frmDotNetInfo.lvClasses.beginUpdate()
    ...
    StartClassFetch(frmDotNetInfo, Image, frmDotNetInfo.edtClassFilter.Text:upper(),
      function()
        ...
        frmDotNetInfo.lvClasses.endUpdate()
...
local function SpawnInjectMethodDialog(frmDotNetInfo)
  ...
  dotNetInfo.SpawnInvokeMethodDialog(Method, getAddressSafe(frmDotNetInfo.comboFieldBaseAddress.Text), 
    ...
    function(classHandle) --OnCreateInstance
     ...
     -- printf("classHandle=%x",classHandle)
      ...
      --if newClass then
      --  printf("known class. It's name is %s",newClass.Name)
      --else
      ...
      if newClass.Methods==nil then
        getClassMethods(newClass)    
      end


      local newInstance,msg=dotNetInfo.createInstanceOfClassDialog(newClass)
...
local function getFieldValue(Field, Address)
  if Address==nil or Address==0 then 
    return getStaticFieldValue(Field)
  end
  
  local a=Address+Field.Offset
...
local function btnLookupInstancesClick(frmDotNetInfo, sender)
  ...
  frmDotNetInfo.scannerThread=createMonoThread(function(t)
    local results={}
...
local function getInstanceAdressFromNode(frmDotNetInfo, node) --in case of multilevel fields
  local Field=getRef(frmDotNetInfo.lvFields2.getNodeDataAsInteger(node))
  if Field==nil then return nil, 'No field data' end
  local Address,err=getAddressFromNode(frmDotNetInfo, node)
    
  if Address==nil then return nil,err end
...
local function lvFields2DblClick(frmDotNetInfo,sender)  
  local Class=frmDotNetInfo.CurrentlyDisplayedClass
  if Class==nil then return end 
 
  local Address=getInstanceAdressFromNode(frmDotNetInfo, sender.FocusedNode)
  if Address==nil then return end  
  

  if sender.FocusedNode==nil then return end
  local Field=getRef(sender.getNodeDataAsInteger(sender.FocusedNode))
  if Field==nil then return end
  ...
  local value=getFieldValue(Field, Address)
   
  
  value=inputQuery(translate('Change value of '..Field.Name), translate('New value:'), value)
  if value then
    setFieldValue(Field, Address, value)
    frmDotNetInfo.tFieldValueUpdater.OnTimer(frmDotNetInfo.tFieldValueUpdater)
  end
...
function miDotNetInfoClick(sender)
  ...
  frmDotNetInfo.comboFieldBaseAddress.PopupMenu=createPopupMenu(frmDotNetInfo)
  local mi=createMenuItem(frmDotNetInfo)
  mi.Caption=translate('Create new instance')
  ...
  frmDotNetInfo.comboFieldBaseAddress.PopupMenu.Items.add(mi)
   ...
   -- frmDotNetInfo.lvFields.Visible=false
    ...
    local columnhandler={
      ...
      [4]=function(node) --value
            local address,err=getInstanceAdressFromNode(frmDotNetInfo, node)
            local Field=getRef(fv.getNodeDataAsInteger(node))
    ...
    frmDotNetInfo.lvMethods.OnCustomDrawItem=function(Sender, ListItem, State)
      ...
      if Method.Static then
        Sender.Canvas.Brush.Color=settings.ColorStaticBackground
      end
      return true
    end
    ...
    frmDotNetInfo.lvFields2.OnExpanding=function(sender, node)
     ...
     -- print("Expanding node")  --debug
      ...
      if Field.VarType~=ELEMENT_TYPE_CLASS then return nil end
    ...
    frmDotNetInfo.lvFields2.OnDblClick=function(sender) lvFields2DblClick(frmDotNetInfo, sender) end
